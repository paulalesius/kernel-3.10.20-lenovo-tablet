...
/*led pwm source 01 from lp855x  00 from panel pwm*/
#define LP8555_PWM_SOURCE  0x1
...
static int lp855x_configure(struct lp855x *lp)
{
	...
	//gpio_request_one(112,GPIOF_OUT_INIT_HIGH,"backlight_en");
	//gpio_direction_output(112,1);
        lp855x_ext_write_byte(0x11,0x3);
...
void i2c_set_backlight_level(int level)
{
	unsigned int config=0;
	config = lp855x_ext_read_byte(0x10);
	gpio_request_one(112,GPIOF_OUT_INIT_HIGH,"blen");
  	gpio_direction_output(112,1);
	printk("drm config:%x",config);
	if(config!=0x4)
	{
		lp855x_ext_write_byte(0x0, 0);
		lp855x_ext_write_byte(0x10, 0x4);
        	lp855x_ext_write_byte(0x11,0x3);
		lp855x_ext_write_byte(0x0, 1);
		lp855x_ext_write_byte(0x10, 0x4);
	}
}
EXPORT_SYMBOL(i2c_set_backlight_level);
