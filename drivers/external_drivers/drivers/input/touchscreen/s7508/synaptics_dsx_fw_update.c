...
#define BLADE2_13_G_GF "blade2_13_g_gff.img"		//blade2 13inch GIS GFF
#define BLADE2_13_O_GM "blade2_13_o_gmf.img"		//blade2 13inch O-Film GMF
...
*/
	if (fwu->ext_data_source) {
		...
		else if((strcmp(fwu->product_id, "Blade13.3") == 0) && (fwu->productinfo1 == 6)){
			strncpy(fwu->image_name, BLADE2_13_G_GF, MAX_IMAGE_NAME_LEN);
		}
		else if((strcmp(fwu->product_id, "Blade13.3") == 0) && (fwu->productinfo1 == 1)){
			strncpy(fwu->image_name, BLADE2_13_O_GM, MAX_IMAGE_NAME_LEN);
		}
		else{
...
/* Add read config id sys I/F. oujf1 */
static ssize_t fwu_sysfs_config_id_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int retval;
