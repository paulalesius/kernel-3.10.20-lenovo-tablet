...
//			if (rmi4_data->hw_if->board_data->x_flip)
            ...
            if(is_dual_display_policy()) {
                input_report_abs(rmi4_data->input_dev,
                        ABS_MT_POSITION_X, 0);
                input_report_abs(rmi4_data->input_dev,
                        ABS_MT_POSITION_Y, 0);

                   // printk("jianming %s dlp_enable" , __func__);
            } else 
            {
                 input_report_abs(rmi4_data->input_dev,
                        ABS_MT_POSITION_X, x);
                input_report_abs(rmi4_data->input_dev,
                  ...
                  //  printk("jianming %s dlp_disenable" , __func__ );
            }
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
			...
			case SYNAPTICS_RMI4_F34:
				f34_fd.ctrl_base_addr = rmi_fd.ctrl_base_addr;
				break;
			}
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	gpio_set_value(PMIC_GPIO1P3_PWR, 0);
	printk("%s set PMIC_GPIO1P3_PWR low, Disable V3P3S_TCH\n", __func__);
...
static int synaptics_rmi4_resume(struct device *dev)
{
	...
	gpio_set_value(PMIC_GPIO1P3_PWR, 1);
	printk("%s Set PMIC_GPIO1P3_PWR high, Enable V3P3S_TCH .\n", __func__);
	msleep(160);
