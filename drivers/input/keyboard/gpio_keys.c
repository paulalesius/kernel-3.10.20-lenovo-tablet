...
static void gpio_keys_gpio_report_event(struct gpio_button_data *bdata)
{
    ...
    printk(KERN_EMERG "button->code = %d, button->desc = %s, type=%d, state = %d\n",
		 ...
		 /*press powre down with dlp not off should not pass to android
				...
				if(((bdata->button->code == KEY_POWER)
						...
						//	bdata->button->code = KEY_PAUSE; //change key event value							
							...
							power_key_down = jiffies_to_msecs(jiffies);
                                                 ...
                                                 if ((dual_display_status.pipea_status !=PIPE_ON) && (bdata->button->code ==KEY_POWER) && dual_display_status.pipeb_status >PIPE_OFF)
                                                 {
                                                       printk(" drm dual_display power key up --main screen is off dont report to up level\n"); 
                                                        return;
                                                 }
                                        ...
                                        if((dual_display_status.pipea_status == PIPE_ON))
                                                          {
                                                              /*  && (dual_display_status.pipeb_status == PIPE_OFF))
                                                                {
                                                                        //simulate_valleyview_irq_handler(1,NULL); 
                                                                }
                                                        else if ((dual_display_status.pipeb_status == PIPE_ON) && dual_display_status.pipea_status==PIPE_ON)
                                                                {
                                                                         //simulate_valleyview_irq_handler(0,NULL); 
                                                                }*/
                                                         }
                                                        else
                                                         {
					                  printk("drm dual_display dlp key up--main screeen is off dont report to up level\n");
					                  //return ;
                                                         }
				}
			}
			...
			if(!!state == 0)
			{
					//this special key will do hotplug the mipib
					if(( bdata->button->code == KEY_DLP))
					{
							printk("drm dual_display debug: dlp switch key!\n");
							/*report the dlp key when eDP is on and dlp is not on; or dlp is on */
							if((dual_display_status.pipea_status == PIPE_ON))
							{/*	&& (dual_display_status.pipeb_status == PIPE_OFF))
								{
									//simulate_valleyview_irq_handler(1,NULL); 
								}
							else if (dual_display_status.pipeb_status == PIPE_ON)
								{
									 //simulate_valleyview_irq_handler(0,NULL); 
								}*/
                                                        }
							else{
                                                                printk("drm dual display dlp key down main screen is off don't report to up level\n");
								//return ;
							}
					}
					...
					if( ((bdata->button->code == KEY_POWER)
						...
						if(!(((power_key_up - power_key_down) > 300) &&
							dual_display_status.pipea_status == PIPE_ON))
							{
								...
								power_key_down = 0;
								power_key_up = 0;
								//bdata->button->code = KEY_PAUSE;
							}
                                                     
                                                 if ((dual_display_status.pipea_status !=PIPE_ON) && (bdata->button->code ==KEY_POWER) && dual_display_status.pipeb_status >PIPE_OFF)
                                                 {
                                                       printk(" drm dual_display power key down --main screen is off dont report to up level\n"); 
                                                        return;
                                                 }
			}
			
		/*when dlp is not off,change back to power key*/
	  /*	if ((bdata->button->code == KEY_PAUSE)
...
static irqreturn_t gpio_keys_gpio_isr(int irq, void *dev_id)
{
    ...
    //key press repeatedly in the debounce time will not invoke the input system
    //XXX implement this for dual display S key.currently borrow the vol up key
    // for the S key don't report to the upper system.
				...
				const struct gpio_keys_button *button)
{
	...
	if(button->code == KEY_POWER)		
		input_set_capability(input, button->type ?: EV_KEY, KEY_PAUSE);
	/*
